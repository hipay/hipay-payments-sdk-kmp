...
// Generates the one constant that carries the SDK version into the artifact, so `source`
// (see SdkIdentity.kt) reports the build a transaction actually came from.
//
// Registered with an output DIRECTORY property so the task provider itself can be handed to
// `kotlin.srcDir(...)` below: Gradle then wires the dependency for EVERY consumer of that source
// directory — the Kotlin compilations and the sources JARs the publication builds. Depending on the
// compile tasks alone is not enough, and fails `publishToMavenLocal` on the sources JAR.
val generateSdkVersion by tasks.registering {
    val versionString = version.toString()
    val outputDir = layout.buildDirectory.dir("generated/sdkVersion")
    inputs.property("version", versionString)
    outputs.dir(outputDir)
    doLast {
        val pkgDir = outputDir.get().asFile.resolve("com/hipay/core")
        pkgDir.mkdirs()
        pkgDir.resolve("SdkVersion.kt").writeText(
            """
            // GENERATED by the `generateSdkVersion` Gradle task — do not edit, do not commit.
            package com.hipay.core

            /** The published version of this SDK build, from `gradle.properties`. */
            internal const val HIPAY_SDK_VERSION: String = "$versionString"
            """.trimIndent() + "\n",
        )
    }
}
...
kotlin {
    ...
    sourceSets {
        commonMain {
            // The generated SDK version: the order's `source` field reports which SDK build produced a
            // transaction, so it has to come from gradle.properties rather than a hand-maintained
            // constant that would silently go stale — losing that visibility is exactly the regression
            // this restores. The TASK is passed, not its directory, so every consumer of this source
            // set (compilations AND the publication's sources JARs) depends on it.
