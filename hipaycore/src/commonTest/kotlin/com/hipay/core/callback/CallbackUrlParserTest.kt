...
class CallbackUrlParserTest {
    ...
    fun parsesAllFiveStatuses() {
        ...
        for ((segment, expected) in cases) {
            val result = CallbackUrlParser.parse(
                "hipaydemo://hipay-payments/gateway/orders/ORDER-42/$segment",
            )
    ...
    fun extractsPercentEncodedOrderId() {
        val result = CallbackUrlParser.parse(
            "hipaydemo://hipay-payments/gateway/orders/ORDER%2042/accept",
        )
        assertEquals("ORDER 42", result.orderId)
    }
    ...
    fun preservesQueryParamsDecoded() {
        val result = CallbackUrlParser.parse(
            "hipaydemo://hipay-payments/gateway/orders/O1/accept" +
    ...
    fun emptyQueryYieldsEmptyMap() {
        assertTrue(
            CallbackUrlParser.parse("hipaydemo://hipay-payments/gateway/orders/O1/cancel")
                .queryParams.isEmpty(),
        )
    }
    ...
    fun acceptsArbitrarySchemes() {
        assertEquals(
            "O1",
            CallbackUrlParser.parse("myapp123://hipay-payments/gateway/orders/O1/accept").orderId,
        )
        assertEquals(
            CallbackStatus.PENDING,
            CallbackUrlParser.parse("com.merchant.app://hipay-payments/gateway/orders/O1/pending").status,
        )
    }
    ...
    fun malformedUrlsThrowTypedValidationError() {
        val malformed = listOf(
            ...
            "hipaydemo://hipay-payments/gateway/orders/accept",          // missing orderId
            "hipaydemo://hipay-payments/something/orders/O1/accept",     // wrong path root
            "hipaydemo://hipay-payments/gateway/orders/O1/approved",     // unknown status
        )
