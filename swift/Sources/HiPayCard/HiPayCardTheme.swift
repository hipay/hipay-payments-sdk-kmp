...
public struct HiPayCardTheme: Equatable, Sendable {
    ...
    /// saved-card sub-line) — mirrors the CMP/Android renderers.
    public var placeholderColor: Color
    /// Tint for monochrome glyphs (neutral card icon, CVV info glyph) and the UNSELECTED
    /// network chips; the selected chip keeps its full-color brand mark (brand logos are
    /// never re-tinted). Pick a tint clearly distinct from the networks' brand colors.
    ...
    /// Field outline width in points.
    public var borderWidth: CGFloat
    /// Field (and saved-card cell) corner radius in points.
